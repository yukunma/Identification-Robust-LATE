...
for (iter in 1 : NUM_RERANDOMIZATION) {
  ...
  for (kdx in 1:max(k)){
    ...
    ##############################################################
    Y_Xbeta22_temp        =   mainY - hat_beta21_beta22[1] -mainX %*% hat_beta21_beta22[3:(dimX+2)];
    Y_beta21_Xbeta22_temp =   mainY-hat_beta21_beta22[1]-hat_beta21_beta22[2]-mainX %*% hat_beta21_beta22[3:(dimX+2)] ;
    Xgamma_temp           =   cbind(1,mainX)%*%hat_gamma;
    beta11_Xbeta12_temp   =   hat_beta11_beta12[1]+hat_beta11_beta12[2]+mainX%*%hat_beta11_beta12[3:(dimX+2)];
    Xbeta12_temp          =  hat_beta11_beta12[1]+mainX%*%hat_beta11_beta12[3:(dimX+2)];
    
    
    
    
    Y_Xbeta22_all        =  rbind(Y_Xbeta22_all, mainY - hat_beta21_beta22[1] -mainX %*% hat_beta21_beta22[3:(dimX+2)]);
    Y_beta21_Xbeta22_all =  rbind(Y_beta21_Xbeta22_all,mainY-hat_beta21_beta22[1]-hat_beta21_beta22[2]-mainX %*% hat_beta21_beta22[3:(dimX+2)]) ;
    Xgamma_all           =  rbind(Xgamma_all, cbind(1,mainX)%*%hat_gamma);
    beta11_Xbeta12_all   =  rbind(beta11_Xbeta12_all, hat_beta11_beta12[1]+hat_beta11_beta12[2]+mainX%*%hat_beta11_beta12[3:(dimX+2)]);
    Xbeta12_all          =  rbind(Xbeta12_all,hat_beta11_beta12[1]+mainX%*%hat_beta11_beta12[3:(dimX+2)]);
    ...
    subsample_denominator = rbind(subsample_denominator,hat_beta11_beta12[2]+matrix(mainZ,,1)*(mainD-beta11_Xbeta12_temp)/plogis(Xgamma_temp)-matrix(1-mainZ,,1)*(mainD-Xbeta12_temp)/(1-plogis(Xgamma_temp)));
    subsample_numerator = rbind(subsample_numerator,hat_beta21_beta22[2]+matrix(mainZ,,1)*Y_beta21_Xbeta22_temp/plogis(Xgamma_temp)-matrix(1-mainZ,,1)*Y_Xbeta22_temp/(1-plogis(Xgamma_temp)));
    
    
    ######just for inference use
    
  }
  subsample_denominator = na.omit(subsample_denominator)
  subsample_numerator   = na.omit(subsample_numerator)
  print(c(iter, mean(subsample_numerator),mean(subsample_denominator),mean(subsample_numerator)/mean(subsample_denominator)));
  theta_hat_temp = sum(subsample_numerator)/sum(subsample_denominator);
  ...
  for(kdx in 1:max(k)){
    ...
    ##############################################################
    main_indices = which( k == kdx );
    curr_list_idx = unique(list_dx[main_indices]);
    CardinalityIk = length(curr_list_idx);
    
    for(i in curr_list_idx){
      
      Gamma = Gamma + CardinalityIk^(-1) * phi_hat[i,1]^2;
      
      J = J + phi_denominator_hat[i,1] / CardinalityIk;
    }#END FOR idx1
...
###end of theta0
  CI1_lower[iter]  = theta_hat_temp - 2*qnorm(0.975)* theta_SD_zeroway;
  CI1_upper[iter]  = theta_hat_temp + 2*qnorm(0.975)* theta_SD_zeroway;
  
  theta_hat[iter]  = theta_hat_temp;
...
for (iter in 1 : NUM_RERANDOMIZATION) {
  ...
  ###end of theta0
  CI2_lower[iter]  = theta_hat_temp - 2*qnorm(0.975)* theta_SD_zeroway;
  CI2_upper[iter]  = theta_hat_temp + 2*qnorm(0.975)* theta_SD_zeroway;
...
for (iter in 1 : NUM_RERANDOMIZATION) {
  ...
  ###end of theta0
  CI3_lower[iter]  = theta_hat_temp - 2*qnorm(0.975)* theta_SD_zeroway;
  CI3_upper[iter]  = theta_hat_temp + 2*qnorm(0.975)* theta_SD_zeroway;
...
for (iter in 1 : NUM_RERANDOMIZATION) {
  ...
  ###end of theta0
  CI4_lower[iter]  = theta_hat_temp - 2*qnorm(0.975)* theta_SD_zeroway;
  CI4_upper[iter]  = theta_hat_temp + 2*qnorm(0.975)* theta_SD_zeroway;
...
for (iter in 1 : NUM_RERANDOMIZATION) {
  ...
  ###end of theta0
  CI5_lower[iter]  = theta_hat_temp - 2*qnorm(0.975)* theta_SD_zeroway;
  CI5_upper[iter]  = theta_hat_temp + 2*qnorm(0.975)* theta_SD_zeroway;
...
for (iter in 1 : NUM_RERANDOMIZATION) {
  ...
  ###end of theta0
  CI6_lower[iter]  = theta_hat_temp - 2*qnorm(0.975)* theta_SD_zeroway;
  CI6_upper[iter]  = theta_hat_temp + 2*qnorm(0.975)* theta_SD_zeroway;
...
for (iter in 1 : NUM_RERANDOMIZATION) {
  ...
  ###end of theta0
  CI7_lower[iter]  = theta_hat_temp - 2*qnorm(0.975)* theta_SD_zeroway;
  CI7_upper[iter]  = theta_hat_temp + 2*qnorm(0.975)* theta_SD_zeroway;
...
for (iter in 1 : NUM_RERANDOMIZATION) {
  ...
  ###end of theta0
  CI8_lower[iter]  = theta_hat_temp - 2*qnorm(0.975)* theta_SD_zeroway;
  CI8_upper[iter]  = theta_hat_temp + 2*qnorm(0.975)* theta_SD_zeroway;
...
for (iter in 1 : NUM_RERANDOMIZATION) {
  ...
  ###end of theta0
  CI9_lower[iter]  = theta_hat_temp - 2*qnorm(0.975)* theta_SD_zeroway;
  CI9_upper[iter]  = theta_hat_temp + 2*qnorm(0.975)* theta_SD_zeroway;
